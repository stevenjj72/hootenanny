...
public:
  ...
  {
    STXXL_MSG("### BigMap: About to check size");
    _checkSize();
    if (_inMemory)
    {
      STXXL_MSG("### BigMap: smallMap");
      return _smallMap[k];
    }
    else
    {
      STXXL_MSG("### BigMap: BigMap");
      return (*_bigMap)[k];
    }
  }
