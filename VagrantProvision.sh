...
# NOTE: These have been changed to pg9.6
...
if ! grep -i --quiet HOOT /etc/postgresql/9.6/main/postgresql.conf; then
echo "### Tuning PostgreSQL..."
sudo -u postgres sed -i.bak s/^max_connections/\#max_connections/ /etc/postgresql/9.6/main/postgresql.conf
sudo -u postgres sed -i.bak s/^shared_buffers/\#shared_buffers/ /etc/postgresql/9.6/main/postgresql.conf
sudo -u postgres bash -c "cat >> /etc/postgresql/9.6/main/postgresql.conf" <<EOT
