...
Set the circular error on items to this value, in meters, by default.
...
=== rubber.sheet.debug.relations

* Data Type: bool
* Default Value: `false`

If set to true then debug relations will be added to nodes. This is a destructive operation that is
only useful when debugging and should not be used during serious conflation.
