...
=== osmapidb.bulk.inserter.file.output.status.update.interval
...
=== osmapidb.bulk.inserter.output.files.copy.location
...
=== osmapidb.bulk.inserter.starting.node.id
...
"osmapidb.bulk.inserter.reserve.record.ids.before.writing.data" is set to true.

=== osmapidb.bulk.inserter.starting.way.id
...
=== osmapidb.bulk.inserter.starting.relation.id
...
=== osmapidb.bulk.inserter.stxxl.map.min.size

* Data Type: long
* Default Value: `10000000`

Size at which the ID mappings storage for the OSM API database bulk inserter switches from a
std::map to an stxxl::map, which is a container optimized for very large amounts of data.  For
debugging purposes only.

=== osmapidb.bulk.inserter.validate.data

* Data Type: bool
* Default Value: `false`

If true, the OSM API database bulk inserter will renumber element IDs, check for duplicated
element IDs, check for invalid way node references, and check for invalid relation member
...
=== translation.debug.in.script

* Data Type: string
* Default Value: ``

The translation script when translating input OGR data when using hoot::TranslationDebugOp.

=== translation.debug.out.script

* Data Type: string
* Default Value: ``

The translation script when translating OSM data to an OGR output when using
hoot::TranslationDebugOp.
